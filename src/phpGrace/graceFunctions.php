 ...
 * @version   2.0.0
 * 异常处理 可以参考：https://www.cnblogs.com/zyf-zhaoyafei/p/6928149.html 介绍的比较详细
 */
...
function graceErrorHandler($code, $message, $file, $line){ 
	if(!PG_SHOWERROR){ exit();/* 关闭错误调试 */ }else{
	    /*开启错误调试*/
 ...
 //捕获错误，设置一个用户自定义的错误处理函数 
set_error_handler("graceErrorHandler");

/**
 * 捕获PHP的错误：Fatal Error、Parse Error等，这个方法是PHP脚本执行结束前最后一个调用的函数 
 * 在脚本结束前判断这次执行是否有错误产生。这时就要借助于一个函数：error_get_last()；
 * 这个函数可以拿到本次执行产生的所有错误。error_get_last();返回的信息：
 *　[type]              - 错误类型
 *　[message] 			- 错误消息
 *　[file]              - 发生错误所在的文件
 *　[line]              - 发生错误所在的行
...
/**
 * 模块配置数据 /phpGrace/configs/ 目录下面的配资
 * @param string $ConfigName  配置文件名称【不要传递 .php 扩展名】默认config，/phpGrace/configs/config.php空文件
 * @param null $key1 配置数据第一层键名称【可选参数 默认全部配置】
 * @param null $key2 配置数据第二层键名称【可选参数 默认 null】
 */
function config($ConfigName='config',$key1=null,$key2=null){
    static $config = null;
    $fileName = PG_IN."configs".PG_DS.$ConfigName.".php"; //模块配置文件路径
    if($config == null){
    ...
    //如果没有传递参数，默认获取全部配置
    if(is_null($key1)){return $config;}
    // 如果有传参数了，且传递的参数存在，返回对应值。参数不存在返回null
    if(is_null($key2)){ if(isset($config[$key1])){ return $config[$key1]; } return null; }
    //判断配置项是否为二级数组格式的
    if(isset($config[$key1][$key2])){ return $config[$key1][$key2]; }
    return null;
...
/**
 * 将数据库里面存储的配置写入到网站具体模块的配置文件configs目录下的具体配置文件
 * (增加修改网站配置的时候调用)
 * @param $data 数据库读取出来的数据
 * @param $configName 需要写入的某个模块名称
 */
function putContent($data,$configName="config"){
    #1.从数据库读取配置，也就是$data
    #2.准备要写入的内容
    $str = "<?php return ".var_export($data,true).";";
    #3.将内容写入 /phpgrace/configs/模块名.php 文件中
    file_put_contents(PG_IN."configs".PG_DS.$configName.".php",$str);
}
